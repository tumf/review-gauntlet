...
- [ ] Add a scoped Textual CSS/layout rule that makes `#agent_panel_container` and `#session_panel_container` stretch to the same height inside `#summary`. (verification: unit - update `tests/test_run_tui.py`; completion condition: source/CSS contract coverage confirms both summary panel containers have the height alignment rule and the rule is scoped to the summary pair)
- [ ] Preserve existing non-summary panel sizing behavior. (verification: unit - update `tests/test_run_tui.py`; completion condition: the common `.panel` style is not changed to force the same height behavior onto finalize/activity/header/footer panels)
- [ ] Preserve Agent/Session summary content and equal width behavior. (verification: unit - run existing `tests/test_run_tui.py`; completion condition: existing human-facing summary, panel title, and semantic style tests continue to pass)
