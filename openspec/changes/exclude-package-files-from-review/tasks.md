...
- [x] Update README default file filtering documentation to describe package manifest and lock file review exclusions while clarifying that inventory may still classify them. (verification: repository evidence - `README.md` default file filtering section names package manifest and lock-file review exclusions and states package files are not removed from general inventory unless another artifact or Git ignore rule excludes them)
...
Archive commitability was blocked by the real archive-gate validation step because `tasks.md:9` used a manual verification note. The note now cites repository-verifiable README evidence instead of manual inspection.
