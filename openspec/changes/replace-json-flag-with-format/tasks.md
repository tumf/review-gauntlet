...
- [x] Replace `inventory` and `plan` parser wiring in `src/review_gauntlet/cli.py` so both commands accept `--format json|text` with default `text`, and no longer accept `--json` (verification: integration - `uv run pytest tests/test_cli.py` includes argument parsing checks that fail for legacy `--json`).
