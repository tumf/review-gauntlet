...
Archive validation previously warned that the `skills/review-gauntlet/SKILL.md` task verification note lacked repository-verifiable evidence. The task note now keeps manual inspection ownership and cites `uv run pytest tests/test_cli_findings.py` as repository-verifiable integration evidence for the `--all` / `--all-findings` distinction.
