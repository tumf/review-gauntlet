...
- [x] Define a typed run-turn continuation schema and validators in a new module `src/review_gauntlet/continuation.py`.
...
- [x] Add `verdict_grace_seconds` field to `CommandAdapterConfig` in `src/review_gauntlet/config.py`.
...
- [x] Render continuation-file instructions into file-scoped ready prompts in `_build_file_scoped_ready_prompt` (`src/review_gauntlet/cli.py`, line 1516).
...
- [x] Inject compact previous-turn context from an existing continuation JSON file into the next prompt.
...
- [x] Add continuation verdict file monitoring to the subprocess polling loop in `_run_session_command_step` (`src/review_gauntlet/cli.py`, line 1983).
...
- [x] Extend `SessionCommandResult` in `src/review_gauntlet/run_controller.py` with `verdict_metadata: dict[str, object] | None = None`.
...
- [x] Add verdict failure reasons to `_agent_status_from_failure_reason` in `src/review_gauntlet/run_controller.py` (line 620).
...
- [x] Map continuation verdict outcomes to run control semantics in `RunController.run()` (`src/review_gauntlet/run_controller.py`, line 307).
...
- [x] Detect no-progress continuation outcomes for targeted actionable work.
...
- [x] Persist verdict artifacts in `_persist_session_command_artifacts` (`src/review_gauntlet/cli.py`, line 2047).
...
- [x] Preserve existing review adapter, timeout, and TUI behavior outside continuation-aware run turns.
