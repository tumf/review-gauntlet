...
- [x] Preserve existing dirty-git and true no-task behavior. Completion condition: existing ready tests for dirty finalize blockers and non-commit/non-drift blockers continue to pass without broadening dirty blocker handling. (verification: unit - `uv run pytest tests/test_cli_ready.py`)
