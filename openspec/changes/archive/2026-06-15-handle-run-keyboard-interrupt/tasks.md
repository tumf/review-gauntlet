...
- [x] Keep existing run outcomes unchanged. (verification: integration - run existing `tests/test_cli.py` run scenarios plus any `tests/test_run_controller.py` scenarios for command failure, no-ready-task, max-step exhaustion, and successful finalization; completion condition: only the interrupt path changes behavior.)
