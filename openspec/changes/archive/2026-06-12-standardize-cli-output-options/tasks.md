...
- [x] Limit `--audience {human,agent}` registration to the `review` command and remove it from non-progress commands. (verification: unit tests in `tests/test_cli.py`, `tests/test_cli_findings.py`, and `tests/test_review_progress.py` prove `review --help` contains `--audience`, while `init`, `status`, `mark`, `finalize`, and `findings` reject `--audience agent`)
- [x] Preserve existing output contracts while renaming the text mode: JSON payloads remain parseable, default text output remains human-readable, and `report --format text` emits the existing Markdown-style report body. (verification: integration tests in `tests/test_cli.py` and `tests/test_cli_findings.py` execute representative commands and parse JSON or assert existing text markers)
