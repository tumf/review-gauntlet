...
- [x] Update the `review-sessions` spec delta to state that Header omits timeout and Agent summary retains timeout. (verification: integration - `openspec/changes/remove-timeout-from-run-tui-header/specs/review-sessions/spec.md` contains the canonical Header/Agent requirements and `cflx openspec validate remove-timeout-from-run-tui-header --strict` passes)
