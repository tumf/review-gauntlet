...
<!-- Expected canonical result after archive: run failure semantics distinguish `quiet_timeout`, overall `timeout`, invalid verdicts, and no-progress turns; no-progress turns retry with diagnostics while bounded by max steps. -->
