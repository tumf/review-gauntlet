...
def _create_session_with_findings(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    capsys.readouterr()
    session_id = json.loads((tmp_path / ".review-gauntlet" / "active-session.json").read_text())["session_id"]
...
def test_findings_filters_by_mark_and_path(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _create_session_with_findings(tmp_path, capsys)
